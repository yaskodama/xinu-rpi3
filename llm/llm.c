...
#include "uart.h"
#include "kmalloc.h"
#include "thread.h"   /* Pi3 は proc.h ではなく thread.h */
#include <stdio.h>

/* Pi4 の uart_putc / uart_puts は Pi3 に無い。シェル版 cmd_llm でしか使わないので
   kprintf に寄せる。推論本体（llm_run）はこれらを使わない。 */
static void uart_putc(char c)        { kprintf("%c", c); }
static void uart_puts(const char *s) { kprintf("%s", s); }
...
/* ARM32(soft-float) 向け: AArch64 の fsqrt 命令が使えないので
   ニュートン法で置き換える。精度は RMSNorm に十分。 */
static float l_sqrtf(float x)
{
    float r; int i;
...
static double l_exp(double x)
{
    ...
    /* ARM32 では unsigned long は 32 ビットしかない。AArch64 のまま
       `unsigned long` で << 52 すると未定義動作になり、しかも共用体の
       上位 4 バイトが書かれず d が壊れる。両方の環境で 64 ビットの
       unsigned long long を使う。 */
    union { unsigned long long u; double d; } sc;
    /* 32/64 ビットの取り違えを次はビルドで止める（ARM32 で一度やった）。 */
...
/* シェルから呼ぶための薄い包み。実測（何秒かかるか）をここで採る。 */
shellcmd xsh_llm(int nargs, char *args[])
{
    return (shellcmd)cmd_llm(nargs, args);
}
