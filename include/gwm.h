// include/gwm.h — minimal window manager for the HDMI framebuffer
// (ported from xinu-rpi5 wm.h for the Pi 3 / arm-rpi3 platform).
//
...
#ifndef XINU_RPI3_GWM_H
#define XINU_RPI3_GWM_H
 ...
 * iteration, before window repainting.  Pass NULL to detach. */
 ...
 * scroll is wired).  Pass 0 to stop. */
void wm_set_autopan(int on);

/* On-screen shell window.  gwin_shell_window_open() adds (idempotently)
 * a "Shell" window to the desktop and resets its text ring; the windowed
 * xsh shell's stdout/stderr is fed in one character at a time via
 * gwin_shell_record() (from the GWINCON0 device's putc).  Rendering is
 * incremental (dirty rows only) so it never flickers the framebuffer. */
int  gwin_shell_window_open(void);
void gwin_shell_record(char c);

#endif /* XINU_RPI3_GWM_H */
