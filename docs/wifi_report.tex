...
\subsection{2.4\,GHz / 5\,GHz 両バンドでの検証}
同一ルータの 2.4\,GHz(\texttt{0C73298EF82D-2G})と 5\,GHz(\texttt{0C73298EF82D-5G}、パスフレーズ共通)
の双方で、接続・DHCP・ping を確認した（表 \ref{tab:ping}）。5\,GHz 側の AP はチャネル 106
（\texttt{chanspec=0xe06a}）という\textbf{DFS チャネル}に在ったが、スキャンで得たファームウェアの
chanspec をそのまま directed join に渡す方式のため、chanspec を手で組み立てることなく 5\,GHz DFS
チャネルへもそのまま接続できた。なお本ドライバの join は毎回 \texttt{WLC\_DOWN} で張り直すため、
バンドの切り替えに明示的な切断操作は不要である（2.4\,GHz から 5\,GHz へは join するだけで切り替わる）。
以前 5\,GHz が接続できなかったのは第 4・5 の壁（構造体・フロー制御）が原因であり、5\,GHz 固有の
問題ではなかったことも確認された。

\begin{table}[h]
\centering
\caption{実機 ping 結果（WiFi 経由、両バンド、定常状態）}
\label{tab:ping}
\small
\begin{tabular}{@{}lccc@{}}
\toprule
バンド & チャネル(chanspec) & 受信/送信(ロス) & RTT 平均(最小/最大) \\
\midrule
2.4\,GHz & ch6 (\texttt{0x1006})  & 10/10 (0.0\,\%)  & 15.3 (11.5/28.0)\,ms \\
5\,GHz   & ch106 DFS (\texttt{0xe06a}) & 7/8 (12.5\,\%)$^{\dagger}$ & 18.4 (15.6/22.2)\,ms \\
\bottomrule
\end{tabular}

\vspace{2pt}
{\footnotesize $^{\dagger}$ 5\,GHz の欠損 1 パケットは ARP 解決中の settling（\texttt{Host is down}）によるもので、
解決後は連続応答（7/7）。2.4\,GHz も初回試行では同様に最初の数発が settling で欠ける。}
\end{table}

\begin{lstlisting}[numbers=none,caption={実機 ping 出力（2.4\,GHz、定常 10 パケット）}]
...
\begin{lstlisting}[numbers=none,caption={実機 ping 出力（5\,GHz DFS ch106、抜粋）}]
...
\section{クライアント側ネットワーク：ping と NTP 時刻取得}
応答（responder）だけでなく、能動的なクライアント機能も実装した。\texttt{wifi\_arp\_resolve()}
で次ホップの MAC を解決し（宛先が同一サブネットならそのホストを、サブネット外ならデフォルト
ゲートウェイを ARP）、その上に ICMP echo クライアント \texttt{wifi\_ping()} と NTP クライアント
\texttt{wifi\_ntp()}（UDP/123）を載せた。サブネット外宛はゲートウェイ経由でルーティングするため、
家庭用ルータや iPhone テザリングの NAT を越えて\textbf{インターネットへ到達}できる。

実機（自宅ルータ 5\,GHz 接続）での結果を表 \ref{tab:client} に示す。NTP は Cloudflare の
公開サーバ（\texttt{162.159.200.123}）へ問い合わせ、応答の送信タイムスタンプ（NTP 1900 年起点）を
Unix 時刻（1970 年起点）へ変換し、JST（UTC+9）の暦日時へ整形した。

\begin{table}[h]
\centering
\caption{クライアント側ネットワーク（自宅ルータ 5\,GHz 経由）}
\label{tab:client}
\small
\begin{tabular}{@{}ll@{}}
\toprule
...
ping 8.8.8.8（インターネット） & 4/4 応答（NAT 越えでインターネット到達） \\
NTP 162.159.200.123 & unix=1780366848 \\
\quad $\to$ JST 整形 & \textbf{2026-06-02 11:20:48} \\
\bottomrule
\end{tabular}
\end{table}

\begin{lstlisting}[numbers=none,caption={NTP 時刻取得（実機トレース）}]
[wifi] === NTP 162.159.200.123 ===
[wifi] *** NTP unix=1780366848  JST=2026-06-02 11:20:48 ***
\end{lstlisting}

iPhone テザリング経由でも ping 8.8.8.8 は 3/4 応答でインターネット到達を確認したが、NTP（UDP/123）は
携帯キャリアでフィルタされる可能性があり、自宅ルータ経由が確実であった。なお NTP の初回問い合わせは
ゲートウェイの ARP 解決が間に合わず取りこぼすことがあるため、ARP がキャッシュされた後に成功する。
本ドライバの WLAN データ経路は UDP/ICMP/ARP までで TCP は未実装のため、TLS を要する HTTPS
（例：一般的なポータルサイト）の取得は範囲外である。
...
WPA2-PSK 接続 $\to$ DHCP $\to$ ping 疎通 $\to$ インターネット到達 $\to$ NTP による現在時刻取得}
までを実機で達成した。鍵となったのは、(i)在荷サプリカント付きファームウェアの選択、(ii)ワイヤ
構造体のサイズ・アラインメントの厳密な一致、(iii)SDPCM 送信クレジット制御の遵守、の 3 点であり、
いずれも実機ログによる地道な診断で特定した。最終的に、自作ドライバは家庭用ルータ／iPhone テザリングの
NAT を越えてインターネットに到達し、NTP サーバから \textbf{JST 2026-06-02 11:20:48} を取得した。

残課題として、TCP を含む上位プロトコル（および TLS／HTTPS）、再接続・ローミング、複数 AP の
永続設定、スキャン一覧からブラウザで選択・接続する UI などが挙げられる。
