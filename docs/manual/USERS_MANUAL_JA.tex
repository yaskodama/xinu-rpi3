...
\subsection{実機 (Raspberry Pi 3 B+) での実行}
QEMU だけでなく、実機の Raspberry Pi 3 B+ (BCM2837) でもこのワークベンチを動かせ
ます。実機向けは専用プラットフォーム \verb|arm-rpi3| でビルドし、microSD カードに
カーネルを焼いて起動します。
...
\end{code}
共有ヘッダ (例: \verb|include/thread.h|) を変更した場合、\verb|make clean| では
\verb|libxc| が再ビルドされません。\verb|printf| 系がおかしくなったら強制再ビルド:
\verb|rm -f lib/libxc/*.o lib/libxc.a && make PLATFORM=arm-rpi3|。

\textbf{SD カードに必要なもの。} Pi 3 のファームウェアは SD の FAT32 パーティション
から起動します。Broadcom ファームウェア (\verb|bootcode.bin|, \verb|start.elf|,
\verb|fixup.dat| --- Raspberry Pi OS の boot パーティションからコピー。本リポジトリ
には含まれません)、\verb|kernel.img| (ビルドした \verb|xinu.boot| をリネーム)、
\verb|config.txt| (\verb|compile/sdcard-rpi3/config.txt|) を置きます。主な
\verb|config.txt| の設定:
\begin{center}\footnotesize
\begin{tabular}{@{}l p{8.0cm}@{}}
\toprule \textbf{行} & \textbf{意味}\\ \midrule
\verb|arm_64bit=0|           & 32-bit (AArch32) で起動\\
\verb|kernel=kernel.img|     & 起動カーネル名\\
\verb|kernel_address=0x8000| & ロードアドレス (0x8000)\\
\verb|enable_uart=1|         & UART を有効化\\
\verb|dtoverlay=disable-bt|  & PL011 (UART0) を GPIO14/15 に配線 (Bluetooth 無効化)\\
\verb|init_uart_clock=48000000| & PL011 基準クロック 48 MHz (115200 8N1 用)\\
\verb|hdmi_force_hotplug=1|  & モニタ未検出でも HDMI 出力を強制\\
\bottomrule
\end{tabular}
\end{center}

\textbf{SD カードの作り方 (方法 A --- FAT32 で新規作成、推奨):}
\begin{code}
...
\end{code}
\textbf{方法 B --- 既存の Raspberry Pi OS カードを再利用 (フォーマット不要):} その
boot パーティション (firmware は既に在る) に \verb|kernel.img| と本リポジトリの
\verb|config.txt| だけをコピーします (\verb|config.txt| が \verb|kernel=kernel.img|
を指定)。

\textbf{シリアルコンソールの配線。} 3.3V の USB-TTL シリアル変換を使います
(\emph{アダプタの 5V は接続しない})。GND \textrightarrow{} pin 6、アダプタ RX
\textrightarrow{} pin 8 (GPIO14/TXD)、アダプタ TX \textrightarrow{} pin 10
(GPIO15/RXD)。115200 8N1 で開く: \verb|screen /dev/tty.usbserial-XXXX 115200|。

\textbf{起動。} SD を Pi 3 に挿し電源 ON。十数〜40 秒ほどでシリアルに \verb|xsh$|
プロンプトが出ます (gwm 入りビルドでは HDMI デスクトップも表示)。あとは QEMU と同じ
く \verb|cc| / \verb|abclc| / \verb|make| / \verb|run| が使えます。Pi 3 B+ ポートの
HDMI デスクトップ (gwm)・WiFi・HTTP サーバ等の操作は \verb|README.md| と
\verb|docs/USERS-GUIDE-JA.md| を参照してください。

\textbf{SD 交換なしのカーネル更新 (\texttt{/kexec})。} \verb|/kexec| 対応ビルドが
既に動いていれば、ネットワーク越しにカーネルを差し替えられます:
\begin{code}
...
\subsection{WiFi への接続 (Pi 3 B+)}
\verb|arm-rpi3| ビルドには BCM43455 WiFi ドライバ (Arasan SDIO 経由) が入っています。
\textbf{起動時に自動接続はしません} --- 毎回 \verb|wifi on| してください。\verb|xsh$|
シェルから:
\begin{center}\footnotesize
\begin{tabular}{@{}l p{9.0cm}@{}}
\toprule \textbf{コマンド} & \textbf{説明}\\ \midrule
\verb|wifi status| & 現在の接続 (SSID + IP/GW) を表示\\
\verb|wifi on| & 対話: スキャン (~30秒、無線が一瞬切れる) \textrightarrow{} AP 一覧 \textrightarrow{} 番号で選択 \textrightarrow{} パスワード (空 = オープン) \textrightarrow{} join + DHCP。成功後 ARP/ICMP 応答を開始し ping 可能に\\
\verb|wifi on <ssid> [pass]| & 非対話 (スキャン/プロンプトなし)。\verb|/shell?cmd=| リモートログイン (stdin なし) 用。pass 省略/空 = オープン\\
\verb|wifi off| & 切断 (BSS ダウン)\\
\verb|wifi-invest| & 「接続済みなのに ping 不通」のとき、応答スレッド再起動・gratuitous ARP・ゲートウェイ ping を通るまで自動リトライ\\
\bottomrule
\end{tabular}
\end{center}
\begin{code}
...
\end{code}
スキャンは一瞬 WiFi を切ります (全チャンネル再同調のため)。再起動後は自動再接続
しないので、起動のたびに \verb|wifi on| してください。有線側の HTTP からも操作でき
ます: \verb|/api/wifi/scan|、\verb|/api/wifi/join?ssid=S&pass=P|、
\verb|/api/wifi/dhcp|、\verb|/api/wifi/ping?ip=A.B.C.D|。

\subsection{複数 Xinu でのメッシュネットワーク (MANET ad-hoc)}
複数の Xinu ボードを \textbf{アクセスポイント無し} で直接つなぎ、ピアツーピアの
メッシュを組めます。802.11 の IBSS (ad-hoc) モード + オンデマンドの AODV ルーティング
を使います (ドローン HIL デモの Pi 4 + Pi 3 ノードと同じ仕組み)。各ノードは同じ
ad-hoc セルに、別々のノード番号で参加します:
\begin{code}
...
\item \verb|<cell-ssid>| --- ad-hoc セル名。\textbf{全ノードで同じ名前・同じ
  チャンネル}にすること。
\item \verb|[channel]| --- 802.11 チャンネル (既定 6)。
\item \verb|[node]| --- 自分のノード番号 (既定 2)。静的 IP \verb|10.0.0.<node>/24|
  を得ます。\textbf{ノードごとに別の番号}を与えること。
\end{itemize}
成功すると \verb|ad-hoc up, ip 10.0.0.<node> (responder running)| と表示され、
\verb|10.0.0.<node>| で ping に応答します。例 --- チャンネル 6 の 3 ノードセル:
\begin{code}
xsh$ wifi adhoc mesh1 6 1      # ノード 1 -> 10.0.0.1
xsh$ wifi adhoc mesh1 6 2      # ノード 2 -> 10.0.0.2
xsh$ wifi adhoc mesh1 6 3      # ノード 3 -> 10.0.0.3
\end{code}
電波の届く範囲のノードは \verb|10.0.0.x| で直接通信できます (同じ IBSS セルに
\verb|10.0.0.x/24| で参加した Mac から \verb|ping 10.0.0.1| 等も可能)。

\textbf{マルチホップ (AODV)。} 宛先が直接届かないとき、最小 AODV モジュール (M13) が
オンデマンドで経路探索します: 発信ノードが RREQ をブロードキャスト (UDP port 654)
し、範囲内の各ノードが中継して逆経路を記録、宛先が RREP を返して順経路
(\verb|route to 10.0.0.x via 10.0.0.y (k hop)|) を確立します。各ノードは応答スレッド
の一部として AODV 制御 (中継 + 返信) を自動で回すので、直接届かないピア宛のトラフィ
ックも中間ノードが転送します (経路表は最大 16 エントリ)。有線 HTTP からノードを
ad-hoc に上げることもできます:
\verb|curl "http://<node-ip>:8080/wifi-adhoc?ssid=mesh1&ch=6&n=2"|。

IBSS/ad-hoc はインフラ (\verb|wifi on|) モードとは独立です (AP に繋がっている場合は
先に \verb|wifi off|)。ad-hoc も再起動後は復元しないので、各ノードで
\verb|wifi adhoc| を再実行してください。
