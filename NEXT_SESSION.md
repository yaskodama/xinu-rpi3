...
- **TCP 実機疎通テスト済 (2026-05-27)**: ✅ **TCP 双方向データ転送は完全動作**。
  ...
  - ★次の手: ICMP_TRACE/TELNET_TRACE を有効化して telnet 内部状態をトレース、または telnetServer
    の 200ms flush ループと子スレッドの telnetWrite/Flush の相互作用を腰を据えて精査
    (osem だけでなく ostart/out バッファの所有権、shell→child の wait/受信タイミング)。
