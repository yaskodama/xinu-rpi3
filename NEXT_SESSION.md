...
**未解決の問題** (legacy mode 0/1/2): mode=0 (parallel) で **3/5 哲学者しか完食しない**。最後の 2 人が隣接フォーク (例: P2/P3 の F3 共有) で livelock。

試した修正 (まだ不十分):
- `fork_denied` 経路に staggered backoff (`sleep(10 + my_id * 8)` ms) 追加
- 5 ms 一律 backoff → スタガー化したが 60s 中に完走しない

### Chandy-Misra 実装済 (commit 5617f6e、2026-06-01 — **未検証、再起動後 deploy 待ち**)

CLASS_ForkCM (7) + CLASS_PhilCM (8) を新設、`mode=3` で起動。各フォークが clean/dirty 状態と pending request slot を持ち、本質的に deadlock-free + 飢餓フリー。

実装ポイント (apps/abcl_program.c):
- `F_ForkCM_holder / _dirty / _request_id` — フォーク状態
- `F_PhilCM_has_lo / _has_hi / _req_lo / _req_hi` — 哲学者ローカルなフォーク所有/リクエスト状態
- ForkCM.request(requester): dirty → 即時 transfer、clean → queue、holder==requester → re-confirm fork_arrived
- ForkCM.release(p): dirty 化 + pending あれば transfer
- PhilCM.try_eat: 両フォーク所有 → eat → release → has_lo/hi 楽観 0 化 + try_eat 再投入
- 初期フォーク分布: 低 ID 哲学者優先 (P0:{F0,F1}, P1:{F2}, P2:{F3}, P3:{F4}, P4:{})、全フォーク dirty 化で第 1 回 transfer 可能

検証コマンド:
```
curl -X POST 'http://192.168.3.50:8080/api/dining/init'
curl -X POST 'http://192.168.3.50:8080/api/dining/start?mode=3&meals=50'
# 30 秒程度待ってから:
curl 'http://192.168.3.50:8080/api/dining/status'
# 期待: n_done=5/5
```

### kexec alignment hazard (commit 5617f6e、2026-06-01 — **重要、文書化必須**)

`upload_slot_data` (apps/webactor.c) を `__attribute__((aligned(16)))` 化。理由:

- `kexec()` は MMU disable 後に LDR r3, [r0], #4 で upload buffer から 0x8000 にコピーする
- MMU off の場合、SCTLR.A=0 でもメモリは Strongly Ordered 扱い → 未整列 LDR は data abort
- バイナリサイズが変わると linker の bss 配置がずれて `upload_slot_data` が奇数アドレスに落ちることがあり、その build だけ kexec が `!EXC A 00007fec` で死ぬ
- 旧 build (326220 B) は偶然整列していて動いていたが、CM 追加で 329420 B に増えた途端に未整列 → 全 kexec 失敗

→ `__attribute__((aligned(16)))` で linker に強制整列させ恒久化。同種の network kernel-update を扱う他の buffer も将来増えるなら同じ対策が必要。
...
**重要**: SD 上の旧 build は `upload_slot_data` がたまたま整列していて kexec 動作可。新 build (commit 5617f6e、329420 B) は CM 追加で +3 KB → 旧 build から `/upload` + `/kexec` で network update 可能。整列 attribute は新 build に commit 済なので、これ以降のサイズ変化に対して恒久的に kexec 安全。
...
### 中優先 (= 次セッションで取り組むべき)
- **Dining mode=3 (Chandy-Misra) 実機検証** ★ 最優先 (commit 5617f6e)
  - 手順: `make PLATFORM=arm-rpi3` → `curl --data-binary @xinu.boot http://192.168.3.50:8080/upload` → `curl -X POST .../kexec`
  - 期待: kexec が `!EXC A` で死なずに新 kernel boot 成功 (alignment fix で恒久対応)
  - その後: `curl -X POST .../api/dining/init`, `.../api/dining/start?mode=3&meals=50`, `.../api/dining/status` polling で **n_done=5/5** を確認
  - 5/5 確認できたら `tools/dining_bench.py` の mode=3 ベンチ + `tools/build_dining_report.py` で PDF 比較レポート (mode 0/1/2/3 各 3 試行)
  - もし mode=3 でも問題あれば: 副次的な race を疑う (PhilCM_try_eat の楽観 has_lo/hi 0 化、fork_lost/arrived の ordering)
- **Legacy mode 0/1/2 の livelock** は CM が動けば後回し OK (歴史的調査価値あり)
