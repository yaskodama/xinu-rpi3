...
- **TCP 実機疎通テスト済 (2026-05-27)**: ✅ **TCP 双方向データ転送は完全動作**。
  ...
  - ★次の手 (これが効率的): **Mac で telnet TCP ストリームを tcpdump** しながら telnet で
    `help` を打ち、**Pi が出力バイトを送出しているか**を直接見る (`sudo tcpdump -ni en0 -X
    'tcp port 23 and host 192.168.3.50'`)。送出されていれば受信側、されていなければ
    telnetWrite/flush。fresh boot の初回接続で行う(サーバ枯渇前)。接続枯渇 ❷ は別途
    telnetServer の accept ループ/シェル EOF-exit を実機トレースで確認。
  - 注: `route/memstat/ps` 等の無出力はシリアルでも起きる別件。`help` はシリアルでは実際に
    出力する(初期キャプチャの取りこぼし)。telnet では help 出力も出ない。
