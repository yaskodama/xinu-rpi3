...
- **TCP 実機疎通テスト済 (2026-05-27)**: ✅ **TCP 双方向データ転送は完全動作**。
  ...
  - ★修正方針 (TCP スタック、要・実機テスト): ① `tcpIss()` を RFC793 風に**時刻ベースで大きく増やす**
    (例 `clkticks*250` 等) して ISS を広く分散 ② `telnetServer` の accept ループが RST/失敗接続で
    スピンせず次を accept するよう見直し (stale tcb の teardown) ③ TIME_WAIT を伸ばす。
    ★これは upstream Embedded Xinu TCP の挙動が高速再接続テストで露呈したもの。通常の単発
    telnet では初回接続は成立する (が出力配送は別途未解決の可能性)。
  - 注: `route/memstat/ps` のシリアル無出力は別件。`help` はシリアルでは出力する。
